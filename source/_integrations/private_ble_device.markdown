...
7. Open `btsnoop_hci.log` in [Wireshark](https://www.wireshark.org/download.html) and search ("Apply display filter") for `btsmp.id_resolving_key`.
   1. Troubleshooting: Users have reported success by **repeating the entire above procedure several times**, including toggling Bluetooth off/on and unpairing the secondary device.
   2. Troubleshooting: Users have reported success by searching for `_ws.col.info matches "Sent LE Add Device to Resolving List"` instead.
8. Select a frame with your phone in the "Source" column (for the phone's IRK) or with your phone in the "Destination" column (for the secondary device's IRK). Expand the "Bluetooth Security Manager Protocol." Copy the value of "Identity Resolving Key".
   1. Troubleshooting: If you searched for `_ws.col.info matches "Sent LE Add Device to Resolving List"`, select any frame, expand "Bluetooth HCI Command - LE Add Device to Resolving List" and copy the value of "Local IRK" for the phone's IRK or the value of "Peer IRK" for the secondary device's IRK.
9. The value needs to be converted by reversing the byte order: First reverse the original value and **then swap each character pair** of the reversed value.
   ...
   For example, if the original value is (colons added for clarity) `76`:`3a`:**`f6`**:`c7`:`f7`:`d9`:`4a`:`d6`:`c2`:`62`:`15`:`8e`:`23`:`20`:`54`:`4e`, the IRK to use would be: `4e`:`54`:`20`:`23`:`8e`:`15`:`62`:`c2`:`d6`:`4a`:`d9`:`f7`:`c7`:**`f6`**:`3a`:`76` - note each byte (character pair, such as **`f6`**) stays the same, but their order is reversed.
   
   When submitting the final result into the integration, there should be no colons - submit `4e5420238e1562c2d64ad9f7c7f63a76`.
